...
from PySide6.QtGui import QCursor, QMouseEvent, QColor, QPainter, QIcon
    ...
    def download(self):
        ...
        if self.audio_only.isChecked() and self.best_quality.isChecked():
            ...
            self.show_custom_message(
                ...
                "Using 'Audio Only' mode (ignoring 'Best Quality')."
            )
            
            args += ["-x"]  # Extract audio
            if self.mp4_output.isChecked():
                args += ["--audio-format", "m4a"]  # M4A is the audio format for MP4
            else:
            ...
            # Default behavior if neither is selected
            args += ["-f", "best"]  # Let yt-dlp decide the best single format
